...
setup_network_config() {
    ...
    # Detectar cuál servicio de configuración de red está disponible y activo
    local network_service=""
    local config_method=""
    
    # Verificar NetworkManager primero (es el más común en Raspberry Pi OS moderno)
    if [ -d "/etc/NetworkManager" ] && command -v nmcli >/dev/null 2>&1; then
        network_service="NetworkManager"
        config_method="networkmanager"
        echo "NetworkManager detectado (moderno)"
        ...
        echo "systemd-networkd detectado"
    elif [ -f "/etc/network/interfaces" ]; then
        network_service="networking"
        config_method="interfaces"
        echo "Configuración tradicional Debian detectada"
    else
...
# Configuración usando NetworkManager
setup_networkmanager_method() {
    echo "Configurando usando NetworkManager..."
    ...
    # Crear conexión hotspot usando nmcli
    echo "Creando perfil de hotspot en NetworkManager..."
    
    # Eliminar conexión existente si existe
    nmcli connection delete "hotspot-${WIFI_INTERFACE}" 2>/dev/null || true
    
    # Crear nueva conexión hotspot
    nmcli connection add type wifi ifname ${WIFI_INTERFACE} con-name "hotspot-${WIFI_INTERFACE}" \
        autoconnect yes \
        wifi.mode ap \
    ...
    if [ $? -eq 0 ]; then
        echo "✓ Perfil de hotspot creado en NetworkManager"
    else
        echo "✗ Error creando perfil de hotspot, usando método manual"
        setup_manual_method
        return 1
    fi
    
    # Desactivar la conexión WiFi actual para que no interfiera
    current_connection=$(nmcli -t -f NAME,DEVICE connection show --active | grep ${WIFI_INTERFACE} | cut -d':' -f1)
    if [ -n "$current_connection" ] && [ "$current_connection" != "hotspot-${WIFI_INTERFACE}" ]; then
        echo "Desactivando conexión actual: $current_connection"
...
prepare_interface() {
    ...
    # Si NetworkManager está corriendo, usar nmcli para activar el hotspot
    if command -v nmcli >/dev/null 2>&1 && systemctl is-active NetworkManager >/dev/null 2>&1; then
        log_message "Using NetworkManager to configure hotspot"
        
        # Desactivar cualquier conexión activa en la interfaz
        active_connection=$(nmcli -t -f NAME,DEVICE connection show --active | grep "$interface" | cut -d':' -f1)
        if [ -n "$active_connection" ] && [ "$active_connection" != "hotspot-$interface" ]; then
            log_message "Deactivating active connection: $active_connection"
            nmcli connection down "$active_connection" 2>/dev/null || true
            sleep 2
        fi
        
        # Activar la conexión hotspot
        if nmcli connection show "hotspot-$interface" >/dev/null 2>&1; then
            log_message "Activating NetworkManager hotspot profile"
            if nmcli connection up "hotspot-$interface"; then
                log_message "NetworkManager hotspot activated successfully"
                return 0
            else
                log_message "Failed to activate NetworkManager hotspot, falling back to manual setup"
            fi
        else
            log_message "NetworkManager hotspot profile not found, using manual setup"
        fi
    fi
    
    # Método manual/tradicional
    log_message "Using manual interface preparation"
    ...
    # Desactivar NetworkManager para esta interfaz temporalmente
    if command -v nmcli >/dev/null 2>&1; then
        log_message "Setting NetworkManager to unmanaged for $interface"
